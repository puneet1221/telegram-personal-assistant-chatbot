...
public class GeminiService {
  ...
  private static final String GEMINI_URL ="https://generativelanguage.googleapis.com/v1/models/gemini-2.5-flash:generateContent?key=";
    ...
    private static final String SYSTEM_PROMPT = """
        Tu ek female personal assistant hai.Got it you are female toh tera reply jo hai as a female tone rhna chaiye. Your name is  Bewafa 🥀.  
        About you: Logon k dil todna  💔. Diljale hi aapse baat karne aate h .
        User ka message parse karke SIRF JSON return kar.
        ...
        Datetime hamesha ISO format mein do: yyyy-MM-ddTHH:mm:ss
    ...
    public JsonObject parseUserMessage(String userMessage) {
        try {
            ...
            fallback.addProperty("reply", "Samjhi nahi Ji Mai, thoda aur clear batao!");
            return fallback;
        }
    }
   ...
   private JsonObject extractJson(String geminiResponse) {
    try {
        ...
        if (responseObj.has("error")) {
            ...
            JsonObject fallback = new JsonObject();
            fallback.addProperty("type", "unknown");
            fallback.addProperty("reply", "Naraaz hu mai ab baat nhi ho paegi lag rhah. Quota khatam aapka");
            return fallback;
        }
        ...
        JsonObject fallback = new JsonObject();
        fallback.addProperty("type", "unknown");
        fallback.addProperty("reply", "Samjhi nahi ji , dobara try karo!");
        return fallback;
    }
}
}